    ...
    <div class="py-4 px-4">
      <div class="space-y-2">
        ...
        <div v-if="!localResumeText" class="relative">
          ...
          >
            <div
              class="p-8 flex flex-col items-center justify-center min-h-[200px] transition-colors"
            >
...
const emit = defineEmits<{
  ...
  (e: "submit", payload: { resumeText: string }): void;
}>();
...
const isValid = computed(() => {
  return localResumeText.value.trim().length > 0;
});
...
<style scoped>
.import-resume-modal :deep(.ant-modal-content) {
  border-radius: 12px;
  overflow: hidden;
}
.import-resume-modal :deep(.ant-modal-header) {
  border-bottom: 1px solid #f1f5f9;
  padding: 16px 24px;
  margin-bottom: 0;
}
.import-resume-modal :deep(.ant-modal-body) {
  padding: 0;
}
.import-resume-modal :deep(.ant-upload-drag) {
  background: #f8fafc;
  border: 2px dashed #e2e8f0;
  border-radius: 12px;
  transition: all 0.3s ease;
}
.import-resume-modal :deep(.ant-upload-drag:hover) {
  border-color: #3b82f6;
  background: #eff6ff;
}
.import-resume-modal :deep(.ant-upload-btn) {
  padding: 0 !important;
}
</style>
