...
<template>
  <header class="h-16 bg-white border-b border-gray-200 px-4 flex items-center justify-between fixed top-0 left-0 right-0 z-50 shadow-sm">
    <!-- 左侧：返回和标题 -->
    <div class="flex items-center space-x-4">
      <Button type="text" @click="handleBack" class="!flex !items-center !justify-center">
      ...
      <Dropdown>
        <template #overlay>
          <Menu @click="({ key }) => handleTemplateChange(key as string)">
            <MenuItem key="template1">简约通用模板</MenuItem>
            <MenuItem key="template2">商务专业模板</MenuItem>
            <MenuItem key="template3">创意设计模板</MenuItem>
          </Menu>
        </template>
        <Button>
          当前模板: 简约通用模板
