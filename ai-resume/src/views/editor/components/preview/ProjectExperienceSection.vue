...
<template>
  <div 
    ...
    @click="handleClick"
  >
    <h3 class="text-lg font-bold text-gray-800 border-b border-gray-300 pb-2 mb-3">项目经历</h3>
    <div class="space-y-4">
      <template v-if="data && data.length">
        <div v-for="(project, index) in data" :key="index">
          ...
          <div class="text-sm text-gray-700 mb-1 font-medium">{{ project.description }}</div>
          <p class="text-sm text-gray-600 leading-relaxed whitespace-pre-wrap">{{ project.content }}</p>
        </div>
      </template>
